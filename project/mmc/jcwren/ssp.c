...
#define WAIT_RXFIFO_NOTEMPTY() do { while (!(SSP_SR & 0x10)); } while (0)
...
U32 sspSetClockFreq (U32 frequency)
{
 /* U32 div = SYSTEMPERIPHERIALCLOCK / frequency;
  ...
  div &= ~1;
  SSP_CPSR = div;
  SSP_CR0 |= (SYSTEMPERIPHERIALCLOCK / div -1) << 8;*/
  ...
  is 0x02*/ 
  SSPCPSR = 0x2; 
  return 0;//(SYSTEMPERIPHERIALCLOCK / div);
}
...
void sspInit (void)
{
 ...
 print("spi init\n");
  ...
  IODIR0 = BIT11 | BIT17 | BIT19;  /* SSEL is output */ 
  IODIR0 &= ~BIT18;
  IOSET0 = BIT11;  /* set SSEL to high */ 
