...
static U32 mmcSendCmd (mmcSpiCmdInd_t commandIndex, U32 arg)
{
  ...
  for (i = 9; i && (ch == 0xff); --i) 
  {
	...
	if (ch !=255)
	{ print("xxxx ch != 0xff xxxx");
		printNum(ch);
		}
  }

  if (i == 0) //If time out occurred.
  {
    sspChipSelect (0);
	print("xxTimeOutxx\n");
    return ((U32) -1);
  ...
  switch (mmcCmd [commandIndex].resp) // switch on response type expected by the command.
  {
    case MmcR1b :
      {
        U32 busy;
...
mmcState_t mmcInitMedia (void)
{
  ...
  //
  Tnac = 100;//IDENTIFICATIONMODECLOCK / SD_READ_TIME_OUT;
